...
const EditBooth = () => {
  ...
  useEffect(() => {
    // Fetch current booth data
    axios.get(`http://localhost:4000/api/booths/${id}`)
      .then((res) => {
        const { stallNumber, name, capacity, hall, availability } = res.data;
  ...
  const handleUpdate = (e) => {
    e.preventDefault();
    axios.put(`http://localhost:4000/api/booths/${id}`, {
      stallNumber,
      name,
      capacity,
      hall: hallId,
  ...
  return (
    <div className="expo-form-wrapper">
      <h2 className="expo-form-heading">Edit Booth</h2>
